...
authors = ["Yichao Fu", "Yiming Zhao", "Hao Zhang"]
author = "Yichao Fu, Yiming Zhao, Hao Zhang"
...
[cover]
      ...
      caption = "Demo of speedups by lookahead decoding on  generation. Blue fonts are tokens generated by accepted by the draft model."
+++
...
{{< image src="img/SpeedupvsDraftTokens.jpg" alt="background" width="80%" title="Figure 1: Speedup vs Draft Tokens. Speedup over autoregressive decoding, comparing LR combined with token-level SD (NGram-based) (red line) to SD alone (blue line). Our method is orthogonal to token-level SD and improves the maximum speedup from 1.4× to 2.1×.">}}

Speculative decoding (SD) accelerates language model decoding by using a small drafter model to propose a sequence of future tokens (denoted by $\gamma$), which are then verified in parallel by the larger target model. If the entire $\gamma$-token sequence is accepted, the target model can skip $\gamma$ steps and advance its state by $\gamma + 1$ positions in a single pass, bypassing the usual step-by-step autoregressive process.

However, the speedup of token-level speculative decoding is intrinsically capped. Let $\alpha\in(0,1)$ be the average per-token acceptance rate, $\gamma$ the number of drafted tokens, and $c$ the draft-to-target per-token cost ratio. Under the standard independence approximation, the expected number of target tokens validated by one target forward pass is $1+\alpha+\cdots+\alpha^{\gamma}=\tfrac{1-\alpha^{\gamma+1}}{1-\alpha}$. The resulting expected wall-time improvement factor (Theorem 3.8 in [the speculative decoding paper](https://arxiv.org/abs/2211.17192)) is

$$
S(\alpha,\gamma,c)=\frac{1-\alpha^{\gamma+1}}{(1-\alpha)(\gamma c+1)}.
$$

This makes the cap explicit: the numerator saturates at $1/(1-\alpha)$ as $\gamma\to\infty$, so even in the idealized zero-overhead limit $c\to 0$, the speedup cannot exceed $1/(1-\alpha)$. For any nonzero overhead $c>0$, the denominator grows linearly in $\gamma$, making $S(\alpha,\gamma,c)$ unimodal and leading to a sharp point of diminishing returns: once the block-accept probability $\alpha^{\gamma}$ falls to the scale of the overhead (roughly $\alpha^{\gamma}\!\lesssim\! c$; more precisely $\alpha^{\gamma+1}\!\approx\! c/|\ln\alpha|$), the optimal $\gamma$ is near $\gamma\approx \ln(1/c)/\ln(1/\alpha)$, and increasing $\gamma$ further yields little or even negative benefit.


This limitation is especially problematic for large reasoning models (LRMs), which typically produce long, structured outputs with step-by-step logic. Since SD can only skip a small number of tokens at a time and this does not scale with the total output length, its contribution to end-to-end latency reduction becomes marginal for long-form reasoning tasks.
...
By shifting speculation from the token-level to the step-level, we bypass the primary bottleneck of traditional SD. Instead of being limited by the near-zero probability of guessing a long, exact token sequence, we can now speculatively generate and verify multiple, complete reasoning steps in parallel. The chance of a proposed step being accepted is dramatically higher because it only needs to capture the correct meaning. This allows us to fully leverage a GPU's batch processing power on meaningful, multi-token chunks of logic. Furthermore, this step-level parallelism is complementary to existing methods; token-level speculation can still operate within each verified step, creating two stacked layers of acceleration for a multiplicative speedup.

This leads to the central challenge: how does one actually perform step-level speculative decoding? Unlike the token-level, where verification is a simple rejection sampling against a probability distribution, it's not clear how to verify if a semantically different reasoning step is "correct" enough for the target model to accept.
